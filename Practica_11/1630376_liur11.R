...
df.singrNA<-subset(df.plantes, !is.na(df.plantes$growthform) & df.plantes$growthform%in%var.estudi, select = c(growthform, height)); df.singrNA
...
df.sinlaiNA<-subset(df.plantes, !is.na(df.plantes$LAI), select=c(LAI, rain)); df.sinlaiNA
...
df.sinppNA<-subset(df.plantes, !is.na(df.plantes$NPP), select=c(NPP, rain)); df.sinppNA
...
vioplot(z1,z2,z3,names=c("Carpicorn","Tropical","Cancer"), col=rainbow(10)[10],
        ylab="Densitat vegetació (LAI)", xlab="Hemisferi",
        main="Densitat de vegetació \n per tropics")
